...
if args.contourfill == 1 and args.overlay == 0 and len(args.background) == 0:
    ...
    # plot contour face
    cf = plt.contourf(xx, yy, data, levels[0:size(levels) - 1], cmap=colormap, extend='both', antialiased=True)
    for l in cf.collections:
        l.set_edgecolor('face')
        l.set_linewidth(0.025)

# show ordinary contours by default
cs = plt.contour(xx, yy, data, levels[0:size(levels) - 1], colors=lc, linewidths=lw, linestyles=ls, antialiased=True)
    ...
    if args.norm == 'linear':
        for i in range(0, size(lvl)):
            if lvl[i] != 0 and (abs(lvl[i]) < 1.0e-3 or abs(lvl[i]) > 1.0e3):
                ...
                clabels[i] = ('%f' %
                              (lvl[i] / cscale)).rstrip('0').rstrip('.') + '$\mathregular{\\times 10^{%i}}$' % scalar
            else:
