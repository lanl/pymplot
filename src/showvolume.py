...
from module_message import fatal, error, msg_output, msg_done
...
def run(args, ax=None):
    ...
    octant = args.octant
    if not octant in ['--+', '---', '-+-', '-++']:
        fatal(f"octant must be one of '--+', '---', '-+-', '-++'; got '{args.octant}'")
    if angle1 < 0 or angle1 > 90 or angle2 < 0 or angle2 > 90:
        fatal(f"view angles must be in [0°, 90°]; got '{args.angle}'")
    ...
    if slice1 <= 0 or slice1 >= n1end:
        fatal('slice 1 position out of data range')
    if slice2 <= 0 or slice2 >= n2end:
        fatal('slice 2 position out of data range')
    if slice3 <= 0 or slice3 >= n3end:
        fatal('slice 3 position out of data range')
    ...
    # 3D pyvista: solid block with one corner carved out
    if args.render == '3d':

        import pyvista as pyv

        off_screen = args.outfile is not None
        p = pyv.Plotter(lighting='three lights', off_screen=off_screen)
        p.set_background(color='white')

        N1, N2, N3 = n1end - n1beg, n2end - n2beg, n3end - n3beg

        subdata = data[n1beg:n1end, n2beg:n2end, n3beg:n3end]
        cmin_pv, cmax_pv = set_clip(args, subdata.flatten(), 'fore', dmin, dmax)

        # ImageData: pyvista x=axis3 flipped, y=axis2 flipped, z=axis1 flipped
        xv = np.flip(subdata, (0, 1, 2)).transpose()
        grid = pyv.ImageData()
        grid.dimensions = np.array(xv.shape) + 1
        grid.origin = (0, 0, 0)
        grid.spacing = (d3, d2, d1)
        grid.cell_data['values'] = xv.flatten(order='F').astype(float)

        lx, ly, lz = N3 * d3, N2 * d2, N1 * d1

        no_crop = args.slice1 is None and args.slice2 is None and args.slice3 is None
        if no_crop:
            ...
            # Node boundaries of the cut planes in pyvista world coords.
            # After flip, original local index i maps to pyvista cell j = N-1-i,
            # so the node between the last cut cell and first kept cell is at (N-i)*spacing.
            # octant NaN assignment: octant_1[:s1,:s2,s3:], octant_2[:s1,:s2,:s3],
            #                         octant_3[:s1,s2:,:s3], octant_4[:s1,s2:,s3:]
            # → z cut always at high pyvista z (i1 < s1 → j ≥ N1-s1)
            # → x cut low  (i3 ≥ s3 → j ≤ N3-1-s3): octant_1, octant_4
            # → y cut high (i2 < s2 → j ≥ N2-s2):    octant_1, octant_2
            px_cut = (N3 - s3) * d3
            py_cut = (N2 - s2) * d2
            pz_cut = (N1 - s1) * d1

            x_cut_low = octant_1 or octant_4
            y_cut_hi  = octant_1 or octant_2

            xb_cut = [0, px_cut]    if x_cut_low else [px_cut, lx]
            yb_cut = [py_cut, ly]   if y_cut_hi  else [0, py_cut]
            zb_cut = [pz_cut, lz]

            # Remove the cut corner (invert=True keeps cells OUTSIDE the cut box),
            # then extract the outer surface: exterior walls + 3 interior cut faces.
            remaining = grid.clip_box(xb_cut + yb_cut + zb_cut, invert=True)
            surf = remaining.extract_surface()

        colormap_pv = set_colormap(args)
        p.add_mesh(surf, scalars='values', cmap=colormap_pv, clim=[cmin_pv, cmax_pv],
                   show_scalar_bar=False, ambient=0.5, diffuse=0.5, specular=0.0)

        # match axis scales to size1/size2/size3 proportions
        l1 = max((N1 - 1) * d1, d1)
        l2 = max((N2 - 1) * d2, d2)
        l3 = max((N3 - 1) * d3, d3)
        lmin = min(l1, l2, l3)
        smin = min(size1, size2, size3)
        p.set_scale(xscale=lmin / l3 * size3 / smin,
                    yscale=lmin / l2 * size2 / smin,
                    zscale=lmin / l1 * size1 / smin)

        lmax = max(l1, l2, l3)
        p.set_focus([0.5 * l3, 0.5 * l2, 0.5 * l1])

        c = [abs(float(v)) for v in args.camera.split(',')]
        if no_crop:
            p.camera_position = [-c[2] * lmax, c[1] * lmax, c[0] * lmax]
        else:
            # Place camera facing into the cut corner
            cam_pv_x = -c[2] if x_cut_low else  c[2]
            cam_pv_y =  c[1] if y_cut_hi  else -c[1]
            cam_pv_z =  c[0]
            p.camera_position = [cam_pv_x * lmax, cam_pv_y * lmax, cam_pv_z * lmax]

        if args.outfile is None:
            p.show()
            msg_done()
        else:
            for i in args.outfile[0].split(','):
                ext = os.path.splitext(i)[1].lower()
                if ext in {'.pdf', '.eps', '.svg', '.ps'}:
                    p.save_graphic(i)
                    msg_output(i)
                    ...
                    error('unsupported output format ' + ext + ', skip ' + i)
            msg_done()
        exit()
