...
def run(args, ax=None):
    ...
    takeslice = args.slice1 is not None or args.slice2 is not None or args.slice3 is not None
    ...
    def project_axis(ax, p1x, p1y, p2x, p2y, ticks, tickbeg, tickend, tickd, mtick, xbeg, xend, ns, d, font,
        ...
        if ticks is None:
            ...
            if ns == 1:
                fatal('contour plot requires at least 2 data points along each axis')
            if ns == 2:
    ...
    cb_levels = project_contour(args, ax, fdata13, face13x, face13y, colormap, cmin, cmax, font, mask13)
    ...
    if args.legend and cmin != cmax:
        ...
        # create colorbar using imshow with discrete contour levels
        x0, x1 = min(cx[0], cx[2]), max(cx[0], cx[2])
        y0, y1 = min(cy[0], cy[2]), max(cy[0], cy[2])
        cb_data = np.linspace(cmax, cmin, cp).reshape(-1, 1)
        cb_norm = colors.BoundaryNorm(cb_levels, ncolors=colormap.N)
        im = ax.imshow(cb_data, aspect='auto', extent=[x0, x1, y0, y1],
                       origin='upper', cmap=colormap, norm=cb_norm, zorder=5)
        im.set_clip_on(False)

        # crate colorbar frame
        line = [[(cx[0], cy[0]), (cx[1], cy[1]), (cx[2], cy[2]), (cx[3], cy[3]), (cx[0], cy[0])]]
        line = LineCollection(line, linewidth=1.0, color='k', clip_on=False)
        ...
        if lloc == 'right':
            ...
            majortick = LineCollection(majortick, linewidths=tick_major_width, colors='k', clip_on=False)
            ax.add_collection(majortick)
            ...
            if args.lmtick != 0:
                ...
                minortick = LineCollection(minortick, linewidths=tick_minor_width, colors='k', clip_on=False)
                ax.add_collection(minortick)
