...
from module_message import fatal, warn, msg_input
from math import *
...
def run(args, ax=None):
    ...
    # for ascii file, data can be directly read
    if args.filetype == 'ascii':
        ...
        # shape of input data
        shape = data.shape
        dmin = data[~np.isnan(data)].min() if np.isnan(data).any() else data.min()
        dmax = data[~np.isnan(data)].max() if np.isnan(data).any() else data.max()
    ...
    if args.filetype == 'binary':
        ...
        if n0 == 0:
            fatal('input file is empty or n1 is too large')
        ...
        if args.n1 is None:
            ...
            n = np.array([int(np.floor(n[0] * 1.0 / n2))])
            nps = n[0]
        else:
            n = args.n1[0].split(',')
            n = [int(i) for i in n]
            ...
            if args.n2 is None:
                ...
                if n2 > int(np.floor(n0 * 1.0 / nps)):
                    fatal('column index out of bounds')
            nmax = max(n)

    # check second dimension size
    if n2 < ns:
        fatal(f'data has {n2} columns but plot type {args.plottype} requires at least {ns}')
    ...
    if args.filetype == 'binary':
        ...
        dmin = data[~np.isnan(data)].min() if np.isnan(data).any() else data.min()
        dmax = data[~np.isnan(data)].max() if np.isnan(data).any() else data.max()
        msg_input(infile if isinstance(infile, str) else infile[0], data.shape, dmin, dmax)
    ...
    if args.select is not None:
        ...
        if select.min() < 0 or select.max() > n2 - 1 or len(select) % ns != 0:
            fatal('column selection out of bounds')
        data = data[:, select]
    ...
    if args.plottype == 1:
        ...
        if args.norm1 == 'log':
            if x1beg <= 0 and x1end <= 0:
                fatal('axis 1 limits invalid for log scale: both must be > 0')
            d1 = abs(d1)
            if x1beg <= 0 and x1end > 0:
                warn(f"axis 1 lower bound ≤ 0 for log scale; clamped to {d1:.4e}")
                x1beg = d1
            if x1beg > 0 and x1end <= 0:
                warn(f"axis 1 upper bound ≤ 0 for log scale; clamped to {d1:.4e}")
                x1end = d1
        ...
        if args.norm2 == 'log':
            if x2beg <= 0 and x2end <= 0:
                fatal('axis 2 limits invalid for log scale: both must be > 0')
            d1 = 1.0e-5 * x2end
            if x2beg <= 0 and x2end > 0:
                warn(f"axis 2 lower bound ≤ 0 for log scale; clamped to {d1:.4e}")
                x2beg = d1
            d1 = 1.0e5 * x2beg
            if x2beg > 0 and x2end <= 0:
                warn(f"axis 2 upper bound ≤ 0 for log scale; clamped to {d1:.4e}")
                x2end = d1
    ...
    def define_tick(ticks,
        ...
        if norm == 'log':
            ...
            if ticks is None:
                ...
                if tickbeg is None:
                    if xbeg <= 0:
                        fatal('x1beg is not a valid number')
                    else:
                        tick_beg = np.floor(math.log(xbeg, base))
                else:
                    if float(tickbeg) <= 0:
                        fatal('tick1beg is not a valid number')
                    else:
                        tick_beg = math.log(float(tickbeg), base)
                if tickend is None:
                    if xend <= 0:
                        fatal('x1end is not a valid number')
                    else:
                        tick_end = np.ceil(math.log(xend, base)) + 1
                else:
                    if float(tickend) <= 0:
                        fatal('tick1end is not a valid number')
                    else:
                ...
                if tickd is None:
                    ...
                    if float(tickd) <= 0:
                        fatal('tick1d is not a valid number')
                    else:
                ...
                if mtick != 0:
                    ...
                    for i in range(0, nt - 1):
                        tlog = np.linspace(np.float_power(base, pticks[i]),
                                           np.float_power(base, pticks[i + 1]), mtick + 2)
            ...
            # irregular ticks
                ...
                if mtick != 0:
                    mtick = mtick + 1
                    minor_tick = np.linspace(np.float_power(base, tick[0]),
                                             np.float_power(base, tick[1]), mtick + 1)
                    minor_tick = minor_tick[1:mtick]
                    for i in range(1, len(tick) - 1):
                        t = np.linspace(np.float_power(base, tick[i]),
                                        np.float_power(base, tick[i + 1]), mtick + 1)
    ...
    if args.projection == 'cartesian':
        ...
        if args.direction == 'horizontal':
            ...
            # if tick font size and family not specified, then inherit from axis
            # labels
            ...
            tick1, tick1_label, minor_tick1, scalar1 = define_tick(args.ticks1, args.tick1beg, args.tick1end,
                                                                   ...
                                                                   size1, args.norm1, int(args.base1),
                                                                   args.tick1format)
            ...
            tick2, tick2_label, minor_tick2, scalar2 = define_tick(args.ticks2, args.tick2beg, args.tick2end,
                                                                   ...
                                                                   size2, args.norm2, int(args.base2),
                                                                   args.tick2format)
            ...
            ax.tick_params('both', length=float(args.tickmajorlen), width=float(args.tickmajorwid),
                           which='major')
    ...
    if args.arrow is not None:
        ...
        for i in range(0, nf):
            ...
            if snf % 2 != 0:
                fatal('arrow specification error: need 4 coordinates (x0,y0,x1,y1)')
    ...
    if args.circle is not None:
        ...
        for i in range(0, nf):
            ...
            if snf == 5:
                ...
                fatal('circle specification error: need 3 values (x,y,radius)')
    ...
    if args.polygon is not None:
        ...
        for i in range(0, nf):
            ...
            if snf % 2 == 0:
                ...
                fatal('polygon specification error: need at least 3 coordinate pairs')
            snf = int(snf / 2.0)
